...
def latest_mamba_version() -> int:
    version = get_mamba_version(raise_on_missing=True)
    assert isinstance(version, Version)
    return version.major

DEFAULT_MAMBA_VERSION = "v2" if latest_mamba_version() >= 2 else "v1"
...
def create_test_model(
    ...
    config_params: Dict[str, Any] = {
        # Scale down parameter counts for testing
        'd_model': 256,
        'n_layer': 4,
        'vocab_size': 12,
        'bidirectional': False,
        # Keep false for orthogonality with other RCPS tests
        'rcps': False, 
        'gradient_checkpointing_stride': 1,
        'layer_cfg': {
            'mamba_cfg': {
                'version': DEFAULT_MAMBA_VERSION
            }
        }
    }
    ...
    return CaduceusForMaskedLM(config, dtype=torch.float16).to(device)
